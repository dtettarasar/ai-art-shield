...
def test_load_image_file_unexpected_io_error(mocker):
    ...
    mocker.patch(
        "PIL.Image.open", side_effect=IOError("Simulated unexpected I/O error")
    )
    ...
    assert "An unexpected error occurred while opening the image" in str(
        excinfo.value
    )  # Vérifie le message de ta fonction
...
def test_load_image_file_other_unexpected_exception(mocker):
    ...
    mocker.patch(
        "PIL.Image.open", side_effect=Exception("Something really bad happened!")
    )
    ...
    with pytest.raises(
        IOError
    ) as excinfo:  # Ton code convertit toutes les 'Exception' en 'IOError'
        Img_Data(dummy_existing_file)
...
def test_pil_to_numpy(img_cs50_instance, img_cookie_instance):
    ...
    assert (
        img_cs50_instance.numpy_array.ndim == 3
    )  # Doit être un tableau 3D (hauteur, largeur, canaux)
    assert img_cs50_instance.numpy_array.shape[2] == 3  # Doit avoir 3 canaux (RGB)
    assert (
        img_cs50_instance.numpy_array.dtype == np.uint8
    )  # Doit être de type uint8 (0-255)
...
def test_numpy_to_pil(img_cs50_instance, img_cookie_instance):
    ...
    img_cs50_from_numpy = img_cs50_instance.convert_numpy_to_pil(
        img_cs50_instance.numpy_array
    )
    img_cookie_from_numpy = img_cookie_instance.convert_numpy_to_pil(
        img_cookie_instance.numpy_array
    )
    ...
    assert np.allclose(
        img_cs50_instance.numpy_array.astype(float),
        reconverted_np_cs50.astype(float),
        atol=1,
    )

    reconverted_np_cookie = np.array(img_cookie_from_numpy)
    assert np.allclose(
        img_cookie_instance.numpy_array.astype(float),
        reconverted_np_cookie.astype(float),
        atol=1,
    )
...
def test_dct_watermark_output_properties(img_cs50_instance):
    ...
    watermarked_channel = img_cs50_instance._apply_dct_watermark_to_channel(
        channel_data.copy(), strength, seed
    )
...
def test_dct_watermark_strength_scaling(img_cs50_instance):
    ...
    original_channel = img_np[:, :, 0].astype(
        float
    )  # Garder en float pour plus de précision
    ...
    diff_low = np.mean(
        np.abs(original_channel - watermarked_low_strength.astype(float))
    )
    ...
    diff_high = np.mean(
        np.abs(original_channel - watermarked_high_strength.astype(float))
    )

    # La différence avec une force élevée doit être significativement plus grande
    assert (
        diff_high > diff_low * 2
    )  # Par exemple, au moins deux fois plus grande, à ajuster
...
def test_apply_dct_watermark_returns_numpy_array(
    img_cs50_instance, img_cookie_instance
):
    ...
    protected_img_np_cs50 = img_cs50_instance.apply_dct_watermark(
        img_np_cs50, strength=5.0
    )
    protected_img_np_cookie = img_cookie_instance.apply_dct_watermark(
        img_np_cookie, strength=5.0
    )
...
def test_apply_dct_watermark_modifies_image(img_cs50_instance, img_cookie_instance):
    ...
    assert np.all(protected_img_np_cookie >= 0) and np.all(
        protected_img_np_cookie <= 255
    )


def test_apply_dct_watermark_converts_and_modifies_grayscale_2d_image(
    sample_grayscale_image_np, img_cookie_instance, caplog
):
    ...
    protected_img_np = img_cookie_instance.apply_dct_watermark(
        img_grayscale_np, strength=5.0
    )

    # 1. Vérifie que le message d'avertissement de conversion a été loggé
    assert (
        "Input is a grayscale image. Converting to 3 channels (RGB) for DCT protection."
        in caplog.text
    )
    ...
    original_3_channels = np.stack(
        [img_grayscale_np, img_grayscale_np, img_grayscale_np], axis=-1
    )
...
def test_apply_dct_watermark_unsupported_channel_count(img_cookie_instance):
    ...
    with pytest.raises(
        ValueError,
        match="^Unsupported image format: must be grayscale \\(1-channel\\) or RGB/BGR \\(3-channel\\).$",
    ):
        img_cookie_instance.apply_dct_watermark(
            unsupported_2_channel_image, strength=5.0
        )
...
def test_apply_dct_watermark_converts_and_modifies_grayscale_1_channel_image(
    sample_grayscale_image_np_1_channel, img_cookie_instance, caplog
):
    """
    ...
    protected_img_np = img_cookie_instance.apply_dct_watermark(
        sample_grayscale_image_np_1_channel, strength=5.0
    )

    # 1. Vérifie que le message d'avertissement de conversion a été loggé
    assert (
        "Input is a 1-channel image. Converting to 3 channels (RGB) for DCT protection."
        in caplog.text
    )
...
def test_secure_image_protected_numpy_created_cs50(img_cs50_instance):
    ...
    assert (
        img_cs50_instance.protected_numpy_array.ndim == 3
    )  # Doit être un tableau 3D (hauteur, largeur, canaux)
    assert (
        img_cs50_instance.protected_numpy_array.shape[2] == 3
    )  # Doit avoir 3 canaux (RGB)
    assert (
        img_cs50_instance.protected_numpy_array.dtype == np.uint8
    )  # Doit être de type uint8 (0-255)
...
def test_secure_image_protected_numpy_created_cookie(img_cookie_instance):
    ...
    assert (
        img_cookie_instance.protected_numpy_array.ndim == 3
    )  # Doit être un tableau 3D (hauteur, largeur, canaux)
    assert (
        img_cookie_instance.protected_numpy_array.shape[2] == 3
    )  # Doit avoir 3 canaux (RGB)
    assert (
        img_cookie_instance.protected_numpy_array.dtype == np.uint8
    )  # Doit être de type uint8 (0-255)
...
def test_export_protected_image_value_error(img_cs50_instance):
    ...
    assert "No protected image array found. Apply protection first." in str(
        excinfo.value
    )
