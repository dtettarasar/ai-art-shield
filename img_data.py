...
class Img_Data:
    ...
    def load_file(self):
        ...
        try:
            ...
            logging.info(
                f"Image loaded: '{self.img_path}', Format: {img_pil.format}, Mode: {img_pil.mode}"
            )
            ...
            raise UnidentifiedImageError(
                f"Unable to identify or open image file '{self.img_path}'. Check format or corruption."
            )
    ...
    def convert_pil_to_numpy(self):
        ...
        logging.debug(
            "Image converted from PIL to NumPy array and stored in self.numpy_array."
        )
    ...
    def convert_numpy_to_pil(self, numpy_array: np.ndarray) -> Image.Image:
        ...
        if numpy_array.dtype != np.uint8:
            logging.warning(
                f"NumPy array has dtype {numpy_array.dtype}, converting to np.uint8."
            )
        ...
        try:
            ...
            logging.info(
                f"NumPy array converted to PIL Image. Mode: {pil_image.mode}, Size: {pil_image.size}"
            )
            return pil_image
        except Exception as e:
    ...
    def _apply_dct_watermark_to_channel(
        self, channel_data, strength: float, seed_value
    ):
        """
        ...
        logging.debug(
            f"Centered channel. Min: {np.min(centered_channel)}, Max: {np.max(centered_channel)}"
        )
        ...
        logging.debug(
            f"Watermark generated. Min: {np.min(watermark):.2f}, Max: {np.max(watermark):.2f}"
        )
        ...
        logging.debug(
            f"Watermark added to DCT coefficients. New Coeffs[0,0]: {dct_coeffs[0,0]:.2f}"
        )
        ...
        reconstructed_centered_channel = idct(
        ...
        watermarked_channel = np.clip(
            reconstructed_centered_channel + 128.0, 0, 255
        ).astype(np.uint8)
        logging.debug(
            f"Processed and clipped channel. Min: {np.min(watermarked_channel)}, Max: {np.max(watermarked_channel)}"
        )

        return watermarked_channel

    def apply_dct_watermark(
        self, input_np_array: np.ndarray, strength: float
    ) -> np.ndarray:
        """
        ...
        if processed_image_np.ndim == 2:  # Image en niveaux de gris (H, W)
            ...
            processed_image_np = np.stack(
                [processed_image_np, processed_image_np, processed_image_np], axis=-1
            )
            ...
            processed_image_np.ndim == 3 and processed_image_np.shape[2] == 1
            ...
            logging.error(
                "Input image must have at least 3 channels (RGB/BGR) for multi-channel DCT protection, or be a standard grayscale image."
            )
            raise ValueError(
                "Unsupported image format: must be grayscale (1-channel) or RGB/BGR (3-channel)."
            )
        ...
        for i in range(3):
            ...
            processed_channel = self._apply_dct_watermark_to_channel(
                processed_image_np[:, :, i].astype(float), strength, channel_seed
            )
    ...
    def secure_image(
        self,
        dct_strength: float = 0,
        wavelet_strength: float = 0,
        adversarial_strength: float = 0,
        qr_opacity: float = 0,
    ) -> np.ndarray:
        """
        ...
        if dct_strength > 0:
            ...
            current_protected_image_np = self.apply_dct_watermark(
                current_protected_image_np, strength=dct_strength
            )
    ...
    def export_protected_image(
        self, output_path: str, format: str = None, quality: int = -1
    ):
        """
        ...
        try:
            ...
            if "format" not in save_kwargs:
                ...
                if (
                    ext in Image.registered_extensions()
                ):  # Vérifie si Pillow connaît cette extension
                    save_kwargs["format"] = ext
                else:
                    logging.warning(
                        f"Unknown format for extension '{ext}', using default Pillow behavior."
                    )
            ...
            raise IOError(
                f"Failed to convert or save image due to data issues: {e}"
            ) from e

        except (
            Exception
        ) as e:  # Capture toute autre erreur générique de sauvegarde (permissions, disque plein, etc.)

            logging.error(
                f"An unexpected error occurred while saving the image to '{output_path}': {e}"
            )
